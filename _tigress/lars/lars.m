function [beta, A, mu, C, c, gamma] = lars(X, Y, option,L)
...
while nVars < L,
    ...
    if isempty(addVar), break; end    % Early stopping criteria
    if signOK,
        A = [A,addVar(1)]; % Add one variable to active set
        nVars = nVars+1;
    end
    s_A = sign(c(A));
    ...
    invG_A = inv(G_A);      % Improve numeric stability by using Cholesky or (economical) QR decompostion, depending on if G_A or X_A is given
